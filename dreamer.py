...
class DreamerV3:
    def __init__(self, obs_shape, act_dim, is_image, is_discrete, config):
        ...
        self.world_model = WorldModel(
            obs_shape, self.act_dim, is_image, is_discrete, config
        ).to(device)
        self.actor = Actor(
            config.hidden_dim, self.act_dim, self.is_discrete, config
        ).to(device)
        ...
        # Hidden states
        self.h = [
            torch.zeros(1, 1, self.config.hidden_dim, device=device)
    ...
    def reset_hidden_states(self, env_id=None):
        if env_id is None:
            self.h = [
                torch.zeros(1, 1, self.config.hidden_dim, device=device)
                for _ in range(self.config.n_envs)
            ...
            self.h[env_id] = torch.zeros(1, 1, self.config.hidden_dim, device=device)
    ...
    def update_world_model(self):
        ...
        # total loss
        loss_world = (
            recon_loss + reward_loss + discount_loss + self.config.kl_scale * kl_loss
        )
    ...
    def update_actor_and_critic(self):
        ...
        imag_h = torch.zeros(
            1, self.config.batch_size, self.config.hidden_dim, device=device
        )
        ...
        imag_s = gumbel_softmax(
            posterior_logits, tau=self.temperature, hard=False
        ...
        for _ in range(self.config.imagination_horizon):
            ...
            imag_s = gumbel_softmax(
                prior_logits, tau=self.temperature, hard=False
            ).view(self.config.batch_size, -1)
        ...
        imag_states = torch.stack(
            imag_states
        )  # Shape: [imagination_horizon, batch_size, hidden_dim]
        imag_rewards = torch.stack(
            imag_rewards
        )  # Shape: [imagination_horizon, batch_size]
        imag_discounts = torch.stack(
            imag_discounts
        )  # Shape: [imagination_horizon, batch_size]
        imag_action_log_probs = torch.stack(
            imag_action_log_probs
        )  # Shape: [imagination_horizon, batch_size]
        ...
        # value prediction
        value_pred = self.critic(imag_states_flat.detach()).view(
        ...
        value_pred_actor = self.critic(imag_states_flat).view(
            self.config.imagination_horizon, self.config.batch_size
        )
    ...
    def act(self, obs, env_id, reset=False):
        if reset:
            self.reset_hidden_states(env_id)
        ...
        with torch.no_grad():
            ...
            # Use the hidden state for the specific environment
            h = self.h[env_id].squeeze(0)  # Shape: [1, hidden_dim]
            ...
            posterior_sample = gumbel_softmax(
                posterior_logits, tau=self.temperature, hard=False
            ).view(1, -1)
            ...
            # update hidden state for the environment
            if self.is_discrete:
            ...
            _, self.h[env_id] = self.world_model.rnn(
                rnn_input.unsqueeze(1), self.h[env_id]
            )

        return action

    def store_transition(self, env_id, obs, act, rew, next_obs, done):
        if not self.is_image:
            self.obs_normalizer.update(obs)
            obs = self.obs_normalizer.normalize(obs)
            next_obs = self.obs_normalizer.normalize(next_obs)

        self.replay_buffer.store(env_id, obs, act, rew, next_obs, done)
