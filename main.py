...
def twohot_encode(x: torch.Tensor, bins: int, min_val: float, max_val: float) -> torch.Tensor:
    x_symlog = symlog(x)
...
def twohot_decode(twohot: torch.Tensor, bins: int, min_val: float, max_val: float) -> torch.Tensor:
...
class CNNEncoder(nn.Module):
    def __init__(self, obs_shape: Tuple[int, ...], config: Config):
        ...
        layers = []
        channels = [in_channels] + [config.cnn_depth * (2 ** i) for i in range(len(config.cnn_kernels))]
        
        for i, (kernel, stride) in enumerate(zip(config.cnn_kernels, config.cnn_strides)):
            layers.extend([
...
class RSSM(nn.Module):
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            if module.out_features == self.config.stoch_size * self.config.stoch_discrete:
    ...
    def initial_state(self, batch_size: int, device: torch.device) -> Dict[str, torch.Tensor]:
        return {
            ...
            'stoch': torch.zeros(batch_size, self.config.stoch_size, self.config.stoch_discrete, device=device),
        }
    
    def observe(self, obs: torch.Tensor, prev_action: torch.Tensor, prev_state: Dict[str, torch.Tensor]) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor]:
        ...
        deter = self.gru(torch.cat([prev_stoch, prev_action], dim=-1), prev_state['deter'])
        
        prior_logits = self.prior_net(deter)
        prior_logits = prior_logits.reshape(-1, self.config.stoch_size, self.config.stoch_discrete)
        ...
        posterior_logits = posterior_logits.reshape(-1, self.config.stoch_size, self.config.stoch_discrete)
        ...
        kl_balanced = (1 - self.config.kl_balance) * kl_raw.detach() + self.config.kl_balance * kl_raw
        
        return state, prior, kl_balanced
    
    def imagine(self, prev_action: torch.Tensor, prev_state: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        prev_stoch = prev_state['stoch'].reshape(prev_state['stoch'].shape[0], -1)
        deter = self.gru(torch.cat([prev_stoch, prev_action], dim=-1), prev_state['deter'])
...
class Actor(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, config: Config, discrete: bool = False):
        super().__init__()
        self.config = config
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            ...
            elif hasattr(self, 'mean_head') and (module is self.mean_head or module is self.std_head):
                nn.init.xavier_uniform_(module.weight, gain=0.01)
            else:
                nn.init.xavier_normal_(module.weight, gain=1.0)
...
class WorldModel(nn.Module):
    ...
    def forward(self, obs: torch.Tensor, action: torch.Tensor, initial_state: Optional[Dict[str, torch.Tensor]] = None):
        ...
        for t in range(seq_len):
            if t == 0:
                prev_action = torch.zeros(batch_size, action.shape[-1], device=obs.device)
            else:
        ...
        for _ in range(batch_size):
            ...
            for key in batch.keys():
                if key in ['obs', 'action']:
                    seq = np.array([self.data[key][(start + i) % max_idx] for i in range(seq_len)])
                else:
...
class DreamerAgent:
    def __init__(self, obs_shape: Tuple[int, ...], action_dim: int, config: Config, discrete: bool = False):
        self.config = config
        self.discrete = discrete
        self.obs_shape = obs_shape
    ...
    def act(self, obs: np.ndarray, state: Optional[Dict[str, torch.Tensor]] = None, training: bool = True) -> Tuple[np.ndarray, Dict[str, torch.Tensor]]:
        with torch.no_grad():
            ...
            if self.prev_action is None:
                if self.discrete:
                    prev_action = torch.zeros(1, self.actor.head.out_features).to(self.device)
                else:
                    prev_action = torch.zeros(1, self.actor.mean_head.out_features).to(self.device)
            else:
                prev_action = torch.FloatTensor(self.prev_action).unsqueeze(0).to(self.device)
            
            prev_stoch = state['stoch'].reshape(1, -1)
            deter = self.world_model.rssm.gru(
                torch.cat([prev_stoch, prev_action], dim=-1),
                state['deter']
            )
            
            posterior_logits = self.world_model.rssm.posterior_net(torch.cat([deter, embed], dim=-1))
            posterior_logits = posterior_logits.reshape(-1, self.config.stoch_size, self.config.stoch_discrete)
            posterior = OneHotDist(logits=posterior_logits, unimix=self.config.unimix)
    ...
    def train(self, steps: int = 1):
        if len(self.replay_buffer) < self.config.batch_size * self.config.sequence_length:
            return {}
        ...
        for _ in range(steps):
            ...
            with torch.no_grad():
                for param, target_param in zip(self.critic.parameters(), self.target_critic.parameters()):
                    target_param.data = self.config.ema_decay * target_param.data + (1 - self.config.ema_decay) * param.data
    ...
    def _train_world_model(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        states, obs_pred, reward_logits, continue_logits, kls = self.world_model(obs, action)
        
        obs_loss = F.mse_loss(obs_pred, obs.reshape(obs.shape[0], obs.shape[1], -1), reduction='mean')
        
        reward_target = twohot_encode(reward, self.config.twohot_bins, 
                                    self.config.twohot_min, self.config.twohot_max)
        ...
        kl_free = torch.maximum(kls, torch.tensor(self.config.free_nats, device=kls.device))
        kl_loss = kl_free.mean()
        
        prediction_loss = self.config.pred_weight * (obs_loss + reward_loss + continue_loss)
        ...
        with torch.no_grad():
            pred_rewards = twohot_decode(F.softmax(reward_logits, dim=-1), self.config.twohot_bins,
                                       self.config.twohot_min, self.config.twohot_max)
            reward_error = F.mse_loss(pred_rewards, reward, reduction='mean')
            
            continue_acc = ((torch.sigmoid(continue_logits.squeeze(-1)) > 0.5) == (1 - done)).float().mean()
            
            obs_error = F.mse_loss(obs_pred, obs.reshape(obs.shape[0], obs.shape[1], -1), reduction='mean')
    ...
    def _train_actor_critic(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        with torch.no_grad():
            ...
            initial_state = self.world_model.rssm.initial_state(obs.shape[0], obs.device)
    ...
    def _train_critic(self, initial_state: Dict[str, torch.Tensor], batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        with torch.no_grad():
            ...
            for _ in range(self.config.horizon):
                ...
                reward = twohot_decode(reward_probs, self.config.twohot_bins,
                                     self.config.twohot_min, self.config.twohot_max)
            ...
            final_value = twohot_decode(final_value_probs, self.config.twohot_bins,
                                      self.config.twohot_min, self.config.twohot_max)
            ...
            for state in states:
                ...
                value = twohot_decode(value_probs, self.config.twohot_bins,
                                    self.config.twohot_min, self.config.twohot_max)
                values.append(value)
            values = torch.stack(values, dim=1)
            
            bootstrap_values = torch.cat([values[:, 1:], final_value.unsqueeze(1)], dim=1)
        ...
        for t in range(self.config.horizon):
            ...
            target_return = returns[:, t] / max(self.config.return_norm_limit, self.return_normalizer.scale)
            target_twohot = twohot_encode(target_return, self.config.twohot_bins,
                                        self.config.twohot_min, self.config.twohot_max)
            
            loss = -torch.sum(target_twohot * F.log_softmax(value_logits, dim=-1), dim=-1)
        ...
        for i in range(num_replay_batches):
            ...
            with torch.no_grad():
                ...
                state = replay_state
                for h in range(min(self.config.horizon, self.config.sequence_length - t)):
                    feat = self.world_model.decoder.get_feat(state)
                    action_dist = self.actor(feat)
                    action = action_dist.rsample()
                ...
                if len(im_rewards) > 0:
                    ...
                    bootstrap_values = torch.cat([im_values[:, 1:], final_value.unsqueeze(1)], dim=1)
                    im_returns = self._compute_lambda_returns(im_rewards, bootstrap_values, im_continues)
                    
                    replay_return = im_returns[:, 0] / max(self.config.return_norm_limit, self.return_normalizer.scale)
            
            replay_feat = self.world_model.decoder.get_feat(replay_state)
            replay_value_logits = self.critic(replay_feat)
            
            target_twohot = twohot_encode(replay_return, self.config.twohot_bins,
                                        self.config.twohot_min, self.config.twohot_max)
            
            loss = -torch.sum(target_twohot * F.log_softmax(replay_value_logits, dim=-1), dim=-1)
            replay_loss += loss.mean()
        
        if num_replay_batches > 0:
            replay_loss = replay_loss / num_replay_batches
        
        critic_loss = self.config.critic_weight * imagination_loss + self.config.critic_replay_weight * replay_loss
        ...
        with torch.no_grad():
            if len(states) > 0:
                ...
                sample_value = twohot_decode(sample_value_probs, self.config.twohot_bins,
                                           self.config.twohot_min, self.config.twohot_max)
        ...
        return {
            ...
            'critic/mean_return_normalized': (returns / max(self.config.return_norm_limit, self.return_normalizer.scale)).mean().item(),
    ...
    def _train_actor(self, initial_state: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            bootstrap_values = torch.cat([values_stacked[:, 1:], final_value.unsqueeze(1)], dim=1)
            returns = self._compute_lambda_returns(rewards, bootstrap_values, continues)
    ...
    def _compute_lambda_returns(self, rewards: torch.Tensor, values: torch.Tensor, 
                               continues: torch.Tensor) -> torch.Tensor:
        returns = torch.zeros_like(rewards)
        
        if rewards.shape[1] > 0:
            returns[:, -1] = rewards[:, -1] + self.config.gamma * continues[:, -1] * values[:, -1]
...
def train_dreamer(env_name: str, total_steps: int = 1000000) -> Tuple[DreamerAgent, List[float]]:
    ...
    print(f"  Model: deter={config.deter_size}, stoch={config.stoch_size}×{config.stoch_discrete}")
    print(f"  Learning rates: model={config.model_lr}, actor={config.actor_lr}, critic={config.critic_lr}")
    ...
    while total_env_steps < total_steps:
        ...
        if total_env_steps > warmup_steps and total_env_steps % (config.batch_size * config.sequence_length // config.replay_ratio) == 0:
            metrics = agent.train()
            update_count += 1
            ...
            if update_count % 10 == 0:
                ...
                eta = (total_steps - total_env_steps) / steps_per_sec if steps_per_sec > 0 else 0
        ...
        if done:
            ...
            if total_env_steps <= warmup_steps:
                ...
                print(f"\r{warmup_label} {next(spinner)} " +
                      f"Step {total_env_steps}/{warmup_steps}, " +
                      f"Episode {episode_count}, " +
                      f"Reward: {reward_str}", end=" ", flush=True)
...
def main():
    environments = ['CartPole-v1', 'Pendulum-v1', 'BipedalWalker-v3', 'LunarLander-v3', 'CarRacing-v3', 'Ant-v5', 'ALE/Pong-v5']
