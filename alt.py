...
This version implements all the things that are missing in the main one.
1. Optimizer issues (all three)
2. Replay Buffer improvements (online queue, latent states, increase capacity to 2M, optimizations to sampling)
3. BlockGRU
4. Replace LayerNorm with RMSNorm
5. Action Repeat Handling
6. Symlog Transformation for Vector Observations
7. Twohot distribution Improvements
"""
...
def twohot_decode(
    ...
    if positive_mask.any():
        ...
        sorted_positive = positive_indices[
            torch.argsort(torch.abs(bin_centers[positive_indices]))
        ]
        positive_sum = torch.zeros_like(twohot[..., 0])
        for idx in sorted_positive:
            positive_sum = positive_sum + twohot[..., idx] * bin_centers[idx]
    else:
        positive_sum = torch.zeros_like(twohot[..., 0])

    if negative_mask.any():
        negative_indices = negative_mask.nonzero().squeeze(-1)
        sorted_negative = negative_indices[
            torch.argsort(torch.abs(bin_centers[negative_indices]))
        ]
        negative_sum = torch.zeros_like(twohot[..., 0])
        for idx in sorted_negative:
            negative_sum = negative_sum + twohot[..., idx] * bin_centers[idx]
    else:
        negative_sum = torch.zeros_like(twohot[..., 0])

    value_symlog = positive_sum + negative_sum
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                normalized_grad = grad / (v.sqrt() + group["eps"])
                m.mul_(beta1).add_(normalized_grad, alpha=1 - beta1)

                p.data.add_(m, alpha=-group["lr"])
...
def adaptive_gradient_clip(
    ...
    for p in model.parameters():
        if p.grad is None or p.numel() < 2:
            continue
...
class BlockGRU(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h: torch.Tensor) -> torch.Tensor:
        ...
        input_gates = input_gates.reshape(
            batch_size, self.num_blocks, 3 * self.block_size
        )

        h_blocks = h.reshape(batch_size, self.num_blocks, self.block_size)
        new_h_blocks = []
        ...
        for i in range(self.num_blocks):
            ...
            block_gates = (
                input_gates[:, i]
                + torch.matmul(block_h, self.block_weights[i])
                + self.block_biases[i]
            )

            r, z, n = torch.chunk(block_gates, 3, dim=-1)
            r = torch.sigmoid(r)
            z = torch.sigmoid(z)
            n = torch.tanh(n)

            new_block_h = (1 - z) * n + z * block_h
            new_h_blocks.append(new_block_h)

        new_h = torch.stack(new_h_blocks, dim=1).reshape(batch_size, self.hidden_size)
        return new_h
...
class ReplayBuffer:
    def __init__(self, capacity: int):
        ...
        self.data = {
            ...
            "state": np.zeros((capacity,), dtype=object),
        }
        self.idx = 0
        self.full = False
        self.online_queue = deque(maxlen=capacity // 10)
        ...
        if state is not None:
            ...
            for k, v in state.items():
                if torch.is_tensor(v):
                    ...
                    state_to_store[k] = v
            self.data["state"][self.idx] = state_to_store
        else:
            self.data["state"][self.idx] = None
        ...
        self.online_queue.append(
            {
                ...
                "reward": reward,
                "done": done,
                "state": state_to_store if state is not None else None,
            }
        )
    ...
    def update_state(self, idx: int, state: Dict[str, np.ndarray]):
        self.data["state"][idx] = {
            k: v.cpu().numpy() if torch.is_tensor(v) else v for k, v in state.items()
        }
    ...
    def sample(
        ...
        online_batch_size = (
            int(batch_size * online_fraction)
            if len(self.online_queue) >= seq_len
            else 0
        )
        ...
        if online_batch_size > 0 and len(self.online_queue) >= seq_len:
            online_data = list(self.online_queue)
            for _ in range(online_batch_size):
        ...
        for _ in range(replay_batch_size):
            ...
            if valid_start:
                ...
                batch["state"].append(self.data["state"][start])
...
class DreamerV3:
    ...
    def act(
        ...
        with torch.no_grad():
            ...
            # Store state without batch dimension for replay buffer
            self.prev_state = {"deter": deter.squeeze(0), "stoch": stoch.squeeze(0)}
    ...
    def _train_world_model(
        ...
        if not self.world_model.rssm.is_image:
            obs_symlog = symlog(obs.reshape(obs.shape[0], obs.shape[1], -1))
            obs_loss = F.mse_loss(obs_pred, obs_symlog, reduction="mean")
        else:
            obs_loss = F.mse_loss(
            ...
            for i, idx in enumerate(indices):
                if idx >= 0:
                    ...
                    self.replay_buffer.update_state(
                        idx + self.config.sequence_length - 1, final_state
                    )
...
def get_action_repeat(env_name: str) -> int:
    if "Atari" in env_name or "ALE" in env_name:
        return 4
    elif "CarRacing" in env_name or "Pendulum" in env_name:
        return 2
    else:
        return 1
    ...
    while total_env_steps < total_steps:
        ...
        if discrete:
            ...
            agent.replay_buffer.add(
                obs, action_onehot, cum_reward, float(done), agent.prev_state
            )
        else:
            agent.replay_buffer.add(
                obs, action, cum_reward, float(done), agent.prev_state
            )

        episode_reward += cum_reward  # giggity
        obs = next_obs
        total_env_steps += config.action_repeat
    ...
    for env_name in environments:
        try:
            ...
            if os.path.exists(best_model_path):
                ...
                load_msg = colored("📂 Loaded best model for evaluation", Colors.BOLD)
